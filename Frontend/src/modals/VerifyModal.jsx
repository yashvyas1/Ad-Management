...
const VerifyModal = ({ data }) => {
  ...
  const [otpVerify, setOtpVerify] = useState(data?.otp || "");
  const [loading, setLoading] = useState(false); // New loading state

  const handleVerifyEmail = async (resetValues = () => {}) => {
    setOtpVerify("");
    if (!otpVerify) {
      await getRequest(
  ...
  const handleSubmitOTP = async (values, { resetForm }) => {
    ...
    try {
      if ((enteredOTP == otp && email) || (enteredOTP == otpVerify && email)) {
        await new Promise((resolve) => setTimeout(resolve, 2000)); // Simulate a delay
        openModal("ResetPasswordModal", { email });
      } else {
        setOtpError("Invalid OTP");
        resetForm({ otp: ["", "", "", "", "", ""] });
      }
    ...
    <div className="flex items-center justify-center bg-opacity-50 z-50">
      <div className="w-full max-w-[20rem] sm:max-w-[24rem] md:max-w-[36rem] bg-white rounded-xl shadow-xl p-6 sm:p-8 md:p-10 mx-2 sm:mx-4 md:mx-10 relative max-h-[90vh] overflow-y-auto">
        <div className="text-center">
          ...
          <h1 className="text-2xl font-semibold text-[#334168]">
