  ...
  const getData = async () => {
    try {
      const response = await getRequest(
        `/api/admin/impressionsgraph?filter=${filter}`
      );
      if (filter === "week") {
        ...
        response.forEach((item) => {
          const monthIndex = new Date(
            Date.parse(item?.groupKey + " 1, 2020")
          ).getMonth();
        ...
        const impressions = years.map(
          (year) => yearData[year]?.impressions || 0
        );
...
export default DashboardAdsBarChart;
