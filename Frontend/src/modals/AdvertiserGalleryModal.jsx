...
import { postRequestAds } from '@/services/backendAPIsServices';

const AdvertiserGalleryModal = ({ setSelectedImages, preSelectedImages }) => {
    ...
    useEffect(() => {
        if (Array.isArray(preSelectedImages) && preSelectedImages.length > 0) {
            setSelectedImageIds(preSelectedImages.map(image => image.id));
        }
    }, [preSelectedImages]);
    ...
    const handleUpload = async (event) => {
        ...
        if (files && files.length > 0) {
            ...
            if (type === "image") {
                ...
                try {
                    const response = await postRequestAds("api/advertiser/uploadmedia", formData, {
                        headers: {
    ...
    return (
        <div className="w-[70rem] bg-white rounded-lg shadow-xl p-4 mx-2 relative mt-8 mb-4 min-h-[70vh] overflow-y-auto">
            ...
            <h2 className="text-2xl font-semibold text-[#444] mb-4">Gallery</h2>
            ...
            <div className="flex items-center justify-between p-3 w-full mt-4 border rounded-lg bg-gray-50">
                <div className='flex items-center'>
                    ...
                    <div className='flex items-center pl-4'>
                        {selectedImageIds.map((id) => {
                            ...
                            return image ? (
                                image.mediaType === "video" ? (
                                    <div key={id} className="w-16 h-16 flex items-center justify-center bg-gray-200 rounded-md pl-2">
                                    ...
                                    <img
                                        ...
                                        src={image.src}
                                        alt={image.title}
                                        className="w-16 h-16 object-cover rounded-md pl-2"
                                    />
                                )
                            ) : null;
                        })}
                ...
                <button
                    className="w-[10rem] bg-blue-500 text-white p-3 rounded-lg focus:outline-none disabled:opacity-50 text-base font-semibold"
...
export default AdvertiserGalleryModal;
